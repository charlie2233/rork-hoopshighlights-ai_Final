...
- Installed-build preflight helper: run `python3 scripts/check_installed_testflight_build.py --device E5786BB6-0095-5509-8B85-110C0B5CE6D3` once the iPhone is available to confirm the installed TestFlight app is `atrak.charlie.hoopsclips` `1.0.0 (51)` before the real-basketball smoke. See `docs/phase_build51_device_smoke_preflight_2026-07-18.md`.
...
- Current gate: the paired iPhone appears as `available (paired)`. One installed-app metadata read confirmed HoopClips `1.0.0 (49)` is installed, while later reads intermittently lost the CoreDevice app metadata channel. Restore stable device tunneling, install or update to internal TestFlight build `51`, cross the old 15-minute point with the same class of real source, and record every downstream step separately.

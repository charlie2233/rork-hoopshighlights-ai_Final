...
class PipelineQualityTests(unittest.TestCase):
    ...
    def test_analysis_team_status_requires_evidence_for_unknown_and_provider_sources(self) -> None:
        ...
        clips = [
            ...
            _clip(start=11.0, end=15.5, label="Blank source make", combined=0.875, event_center=13.2, auto_keep=True).model_copy(
                update={
                    "teamAttribution": ClipTeamAttribution(
                        teamId="team_dark",
                        label="Dark jerseys",
        ...
        self.assertEqual(annotated[2].teamAttributionStatus, "uncertain")
        self.assertEqual(annotated[3].teamAttributionStatus, "matched")
        self.assertEqual(annotated[4].teamAttributionStatus, "matched")
    ...
    def test_review_trim_reserves_audio_reaction_candidate_for_small_review_set(self) -> None:
        scoring = [
            _clip(
                start=float(index * 5),
                end=float(index * 5 + 4),
        ...
        crowd_pop = _clip(
            ...
            event_center=32.0,
            auto_keep=False,
        ).model_copy(update={"audioScore": 0.99, "motionScore": 0.82, "visualScore": 0.46})

        trimmed = _trim_analysis_clips_for_review([*scoring, crowd_pop], None, max_clips=4)
        labels = [clip.label for clip in trimmed]

        self.assertEqual(len(trimmed), 4)
        self.assertIn("Crowd Reaction", labels)
        self.assertNotIn("Made Shot 3", labels)

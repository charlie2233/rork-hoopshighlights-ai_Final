...
from .model_registry import model_registry_for_settings
...
def run_analysis(job: StoredJob, settings: Settings, source_path: Path) -> CloudAnalysisResult:
    ...
    if detection_provider and settings.detection_provider == "hoopcut":
        ...
        external_pipeline = annotate_external_clips(
            ...
            source_identity=_job_source_identity(job),
        )
    ...
    diagnostics = CloudDiagnostics(
        ...
        usedSemanticRerank=_analysis_used_semantic_rerank(clips),
    ...
    return CloudAnalysisResult(
        ...
        assetStorageKey=job.storage_key,
...
def _job_source_identity(job: StoredJob) -> dict[str, str]:
    raw: dict[str, object | None] = {
        "jobId": job.job_id,
        "requestId": job.request_id,
        "traceId": job.trace_id,
        "uploadTraceId": job.upload_trace_id,
        "inferenceAttemptId": job.inference_attempt_id,
...
def _analysis_used_semantic_rerank(clips: Sequence[CloudClip]) -> bool:
    for clip in clips:
        rerank = clip.provenance.embeddingRerank if clip.provenance is not None else None
        if rerank is None or rerank.status != "applied":
            continue
        if rerank.adapter in {"openclip", "siglip"}:
            return True
    return False
...
def _run_native_candidate_detection(
    ...
    result = run_staged_detection_pipeline(
        windows,
        registry=model_registry_for_settings(settings),
        clip_limit=resolved_limit,

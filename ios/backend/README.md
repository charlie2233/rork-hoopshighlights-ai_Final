...
- `HOOPS_MAX_RETURNED_CLIPS`: high-recall cloud analysis candidate pool returned to iOS and AI Edit, clamped to `8...60` (default `60`)
...
- `HOOPS_TEAM_QUICK_SCAN_CLIP_FRAMES_PER_CLIP`: rich per-candidate clip frames for team ownership, clamped to `1...6` (default `6`)
- `HOOPS_TEAM_QUICK_SCAN_RICH_CANDIDATE_CLIPS`: number of top candidate clips that receive the rich frame budget before the compact tail budget is used (default `60`)
- `HOOPS_TEAM_QUICK_SCAN_MAX_TOTAL_CLIP_FRAMES`: total per-clip frame budget for team quick scan, excluding whole-video context frames (default `720`)
- `HOOPS_TEAM_QUICK_SCAN_MAX_CANDIDATE_CLIPS`: maximum analysis candidates included in team quick scan attribution, clamped to `1...160` (default `160`)

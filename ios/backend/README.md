...
- `HOOPS_TEAM_QUICK_SCAN_RICH_CANDIDATE_CLIPS`: number of top candidate clips that receive the rich frame budget before the compact tail budget is used (default `40`)
- `HOOPS_TEAM_QUICK_SCAN_MAX_TOTAL_CLIP_FRAMES`: total per-clip frame budget for team quick scan, excluding whole-video context frames (default `480`)

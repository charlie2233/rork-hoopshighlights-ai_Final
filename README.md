...
- Current internal-beta status: staging deploy run `29632345235` passed live Worker/direct editing version proof, and the deterministic Worker render smoke passed. Build `57` keeps direct-to-R2 byte progress, shared-session multipart recovery, missing-target renewal, and up to six high-priority Wi-Fi lanes. It also prepares a balanced 720p upload source for recordings at least 12 minutes or 256 MiB, but uses it only when it saves at least 18 percent. Signed upload run `29723139547` and read-only status run `29723517882` prove archived source SHA `15339ce5c4602a61abb562db5c6d4ca97fe39dbb` is available in App Store Connect as internal TestFlight build `1.0.0 (57)`, `VALID`, `IN_BETA_TESTING`, `INTERNAL_ONLY`, and ready for internal testers.
- Current `main` is `72701b2abfd229386cd9353e1056f45019a2c2e1` after PR #117's bounded team-scan batching and PR #118's transfer-relative upload display. Main push runs `29781455716` and `29781456008` passed the cloud preflight and no-secret iOS codecheck. These post-build-57 changes have no newer signed TestFlight binary.
...
- Apple account agreements and certificates are active, and build `57` proved automatic signing, archive, provisioning, upload, processing, and internal TestFlight availability. Apple signing is not the current blocker; see `TESTFLIGHT_BLOCKER.md`.
...
Last launch-gate verification: July 20, 2026.
...
- PR #112: merged at `b1a655eab95e67cb80d18bb3ba8c6be727910dcb`, multipart startup throughput correction with a bounded network-classification warmup, six normal-network lanes, explicit per-host capacity, and high-priority upload tasks.
- PR #113: merged at `701624eb9fed343c43720d060a78e9fcada7df80`, build `56` archive/status guards; this is the exact source SHA archived for build `56`.
- PR #115: merged at `15339ce5c4602a61abb562db5c6d4ca97fe39dbb`, automatic upload-source optimization and build `57`; this is the exact source SHA archived for build `57`.
- PR #117: merged at `b7e318a41e4639bee0a0f7d22919639800a1e9`, bounded team-scan attribution batches and enforced the backend payload ceiling.
- PR #118: merged at `72701b2abfd229386cd9353e1056f45019a2c2e1`, made upload waiting views use the actual byte-transfer percentage without changing upload architecture. No new TestFlight build was created.
- GitHub Actions on merged `main`:
  ...
  - `iOS Internal TestFlight Upload` upload run `29706183087`: success for build `55` on archived source SHA `bc7218c6ca412cdc1241dd74772b8f84a27a2597`; signing capacity, signed archive, metadata/privacy verification, upload, and runner-owned certificate cleanup passed.
  ...
  - `iOS Internal TestFlight Upload` push/codecheck run `29716039765`: success on build `56` merged main SHA `701624eb9fed343c43720d060a78e9fcada7df80`; all 15 focused simulator tests passed.
  - `iOS Internal TestFlight Upload` upload run `29716394858`: success for build `56`; signing capacity, signed archive, metadata/privacy verification, App Store Connect upload, and runner-owned certificate cleanup passed.
  - `iOS Internal TestFlight Upload` status run `29717019768`: success; build `56` is `VALID`, `IN_BETA_TESTING`, `INTERNAL_ONLY`, not expired, and ready for internal testing.
  - `iOS Internal TestFlight Upload` push/codecheck run `29722969229`: success on build `57` merged main SHA `15339ce5c4602a61abb562db5c6d4ca97fe39dbb`; all 16 focused simulator tests passed.
  - `iOS Internal TestFlight Upload` upload run `29723139547`: success for build `57`; signing capacity, signed archive, metadata/privacy verification, App Store Connect upload, and runner-owned certificate cleanup passed.
  - `iOS Internal TestFlight Upload` status run `29723517882`: success; build `57` is `VALID`, `IN_BETA_TESTING`, `INTERNAL_ONLY`, not expired, minimum iOS `17.0`, does not use non-exempt encryption, and is ready for internal testing.
  - `Cloud Edit Deploy Preflight` push run `29781455716`: success on current main `72701b2abfd229386cd9353e1056f45019a2c2e1`.
  - `iOS Internal TestFlight Upload` push/codecheck run `29781456008`: success on current main; internal staging checks, plist validation, and all 16 focused simulator tests passed. Archive, signing, and upload were intentionally skipped.
...
- App Store Connect build proof: `1.0.0 (57)` is available for internal TestFlight testing and matches archived main SHA `15339ce5c4602a61abb562db5c6d4ca97fe39dbb`. Installed phone smoke remains unproven because the trusted iPhone must install/update build `57` from TestFlight before the real-basketball smoke.
- Paired-device observation on July 20: the trusted iPhone 15 Pro is available and still has HoopClips `1.0.0 (53)` installed. TestFlight `4.2.2` is installed and was opened for the update, but build `57` was not yet installed; no build-57 device smoke is claimed.
...
- Build `55` proof: carries build `54` throughput/routing work and renews expired canonical-asset multipart targets while preserving saved completed chunks. Signed upload/status proof passed; installed phone proof remained required for that build.
- Build `56` proof: carries the build `55` recovery path and the PR #112 multipart startup fix. Signed upload/status proof passed; a real iPhone must still prove that a large basketball upload advances beyond the former 15% stall and completes the upload-through-export flow.
- Build `57` proof: automatically prepares a smaller upload source for long or large recordings while preserving the direct R2 multipart path. Signed upload/status proof passed; wall-clock speed and end-to-end behavior still require a real iPhone basketball smoke.
...
- `docs/phase_build56_testflight_proof_2026-07-20.md` - build `56` multipart startup fix, signed upload/status proof, and remaining device gates.
- `docs/phase_build57_testflight_upload_optimization_2026-07-20.md` - build `57` automatic upload-source optimization, signed upload/status proof, and remaining device gates.

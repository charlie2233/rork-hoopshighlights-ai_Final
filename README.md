...
- Current internal-beta status: staging deploy run `29632345235` passed live Worker/direct editing version proof for the unchanged build `50` cloud backend. Build `53` carries the upload recovery and declutter work plus PR #92 AI Edit launch polish. Main codecheck run `29667213808`, upload run `29667373531`, and status run `29667648668` prove current main SHA `55a402b4b6cc48038306af5eed72367adab15bbf` is available in App Store Connect as internal TestFlight build `1.0.0 (53)`, `VALID`, `IN_BETA_TESTING`, `INTERNAL_ONLY`, and ready for internal testers.
...
- Apple account agreements and certificates are active, and build `53` proved automatic signing, archive, provisioning, upload, processing, and internal TestFlight availability. Apple signing is not the current blocker; see `TESTFLIGHT_BLOCKER.md`.
...
Last launch-gate verification: July 18, 2026.
...
- App Store Connect build proof: `1.0.0 (53)` is available for internal TestFlight testing and matches current main SHA `55a402b4b6cc48038306af5eed72367adab15bbf`. Installed phone smoke remains unproven because the trusted iPhone must install/update build `53` from TestFlight before the real-basketball smoke.
...
- Build `53` proof: carries the upload recovery and status declutter plus PR #92 AI Edit studio/progress polish. Signed upload/status proof passed; installed phone proof remains required for current main.

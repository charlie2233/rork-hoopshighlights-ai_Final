...
- Current internal-beta status: staging deploy run `29632345235` passed live Worker/direct editing version proof for the build `50` main SHA. Build `51` is uploaded from main SHA `60eda29b7989e97a93ebdf973c0d80446caa07bf`; upload run `29644918870` passed signed archive, metadata/privacy verification, App Store Connect upload, and runner-owned certificate cleanup. Status run `29645129050` reports `1.0.0 (51)` as `VALID`, `IN_BETA_TESTING`, `INTERNAL_ONLY`, and ready for internal testers.
...
- Apple account agreements and certificates are active, and build `51` proves automatic signing, archive, provisioning, upload, processing, and internal TestFlight availability. Apple signing is not the current blocker; see `TESTFLIGHT_BLOCKER.md`.
...
- App Store Connect build proof: `1.0.0 (51)` is available for internal TestFlight testing; its signed archive, upload, and read-only status workflows passed on merged main.
...
- Build `51` internal TestFlight build: one-hour presigned URLs, a bounded one-hour post-expiry multipart renewal window, foreground background-session reconciliation, selected AI Edit style polish, and twelve focused iOS CI tests. Merge, staging deployment, signed archive/upload, Apple processing, and internal TestFlight availability are proven. Installed phone proof remains required.

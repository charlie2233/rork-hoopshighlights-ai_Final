...
DEFAULT_TRACKID3X3_SOURCE_DOMAIN = "trackid3x3:fixed-camera"
...
def _import_tracking_row(
    ...
    *,
    ...
    source_ref = _first_text(row, "sourceRef", "source_ref", "path", "videoPath", "video_path")
    evidence = _normalize_evidence(row)
    source_label = _first_text(row, "label", "eventLabel", "event_label", "action", "category", "class", "event")
    ball_visible = _first_bool(
        row,
        "ballVisible",
        "ball_visible",
        default=_detection_present(detections, {"ball", "basketball"}) or _tracking_present(row, {"ballTrack", "ballTrackId", "ballCarrierId"}),
    )
    hoop_visible = _first_bool(
        row,
        "hoopVisible",
        "hoop_visible",
        default=_detection_present(detections, {"rim", "hoop", "backboard"}) or _tracking_present(row, {"rim", "hoop", "backboard"}),
    )
    ball_near_rim = _tracking_ball_near_rim(row, detections)
    ball_through_hoop = _first_probability(
        row,
        "ballThroughHoopLikelihood",
        "ball_through_hoop_likelihood",
        default=min(1.0, (ball_near_rim or 0.0) * 0.9) if ball_near_rim is not None else None,
    )
    possession_change = _first_probability(
        row,
        "possessionChangeLikelihood",
        "possession_change_likelihood",
        default=_tracking_possession_change(row, source_label=source_label),
    )
    transition_likelihood = _first_probability(
        row,
        "transitionLikelihood",
        "transition_likelihood",
        default=_tracking_transition_likelihood(row, source_label=source_label),
    )
    ...
    event_family = _first_text(row, "eventFamily", "event_family") or _tracking_event_family(
        source_label,
        detections=detections,
    ...
    outcome = _first_text(row, "outcome", "result") or _tracking_outcome(
        source_label,
        event_family=event_family,
        ball_visible=ball_visible,
        hoop_visible=hoop_visible,
        ball_near_rim=ball_near_rim,
        ball_through_hoop=ball_through_hoop,
    )
    shot_subtype = _first_text(row, "shotSubtype", "shot_subtype") or _tracking_shot_subtype(source_label, event_family=event_family)
    annotation = annotation_template(clip_id=clip_id, source_domain=source_domain)
    annotation.sourceRef = source_ref
    annotation.eventFamily = event_family
    annotation.outcome = outcome
    annotation.shotSubtype = shot_subtype if event_family == "shot_attempt" else None
    annotation.ballVisible = ball_visible
    annotation.hoopVisible = hoop_visible
    annotation.ballNearRim = ball_near_rim
    annotation.ballThroughHoopLikelihood = ball_through_hoop
    ...
    annotation.rawTeacherOutputs = {
        ...
        "evidenceFields": sorted(evidence),
        "importVersion": ANNOTATION_SCHEMA_VERSION,
        "notes": _first_text(row, "teacherNotes", "teacher_notes", "notes")
        or f"{source_dataset} tracking supervision imported into canonical hierarchy.",
        "confidence": teacher_confidence,
    }
    return annotation
...
def _normalize_source_kind(source_kind: str) -> str:
    ...
    aliases = {
        ...
        "trackid3x3-fixedcamera": "trackid3x3-tracking",
    }
...
def _normalize_evidence(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for key in ("sourceLabel", "label", "eventLabel", "event_label", "action", "category", "class"):
        if key in row and row[key] not in (None, ""):
            ...
            break
    for key in ("sourceDomain", "source_domain", "sourceDataset", "source_dataset", "cameraType", "camera_type", "footageType", "footage_type"):
        if key in row and row[key] not in (None, ""):
            evidence[key] = row[key]
...
def _normalize_detections(row: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if raw_detections is None:
        raw_detections = row.get("tracks")
    if raw_detections is None:
        raw_detections = row.get("tracklets")
    if raw_detections is None:
        raw_detections = row.get("playerTracks")
    if raw_detections is None:
        raw_detections = row.get("ballTracks")
    if raw_detections is None:
        return []
...
def _tracking_present(row: Mapping[str, Any], keys: set[str]) -> bool:
    for key in keys:
        if key in row and row[key] not in (None, "", [], {}):
            return True
    return False
...
def _tracking_ball_near_rim(row: Mapping[str, Any], detections: Sequence[Mapping[str, Any]]) -> float | None:
    ...
    if explicit is not None:
        return explicit
    explicit_distance = _first_probability(
        row,
        "ballRimDistance",
        "ball_rim_distance",
        "ballToRimDistance",
    ...
    if explicit_distance is not None:
        return max(0.0, min(1.0, 1.0 - explicit_distance))
    return _compute_ball_near_rim(row, detections)


def _tracking_possession_change(row: Mapping[str, Any], *, source_label: str | None) -> float | None:
    explicit = _first_probability(
        row,
        "possessionChangeLikelihood",
        "possession_change_likelihood",
    ...
    if explicit is not None:
        return explicit
    if _normalize_label(source_label) in {"turnover", "steal", "defensive event"}:
        return 0.85
    if _tracking_present(row, {"ballCarrierChange", "ballCarrierChanged", "carrierChange", "trackSwitches"}):
        return 0.7
    if _tracking_present(row, {"turnover", "steal"}):
        return 0.9
    return 0.0


def _tracking_transition_likelihood(row: Mapping[str, Any], *, source_label: str | None) -> float | None:
    explicit = _first_probability(
        row,
        "transitionLikelihood",
        "transition_likelihood",
    ...
    if explicit is not None:
        return explicit
    if any(token in _normalize_label(source_label) for token in {"fast break", "transition", "full court", "coast to coast"}):
        return 0.9
    if _tracking_present(row, {"transition", "fastBreak", "fast_break", "coastToCoast", "fullCourt"}):
        return 0.8
    speed = _first_probability(row, "ballCarrierSpeed", "ball_carrier_speed", "transitionSpeedScore", "transition_speed_score")
...
def _tracking_event_family(
    ...
    *,
    ...
    possession_change: float | None,
    transition_likelihood: float | None,
) -> str:
    normalized = _normalize_label(label)
    if any(token in normalized for token in {"dead ball", "deadball", "replay", "celebration", "inbound", "dribble only", "camera pan", "half court setup", "warmup"}):
        return "other"
    if any(token in normalized for token in {"block", "steal", "defense"}):
        return "defensive_event" if "block" in normalized else "turnover"
    if any(token in normalized for token in {"turnover"}):
        return "turnover"
    if any(token in normalized for token in {"fast break", "transition", "runout", "breakaway", "coast to coast"}):
        return "transition"
    if _detection_present(detections, {"block"}):
        return "defensive_event"
    if _detection_present(detections, {"steal", "turnover"}):
        return "turnover"
    if (transition_likelihood or 0.0) >= 0.65 or (possession_change or 0.0) >= 0.7:
        return "transition"
    if ball_visible and hoop_visible and ball_near_rim is not None and ball_near_rim >= 0.3:
        return "shot_attempt"
    return "other"
...
def _tracking_outcome(
    ...
    *,
    ...
    hoop_visible: bool,
    ball_near_rim: float | None,
    ball_through_hoop: float | None,
) -> str:
    normalized = _normalize_label(label)
    if any(token in normalized for token in {"blocked", "block"}):
        return "blocked"
    if any(token in normalized for token in {"miss", "rim out", "airball", "no good"}):
        return "missed"
    if any(token in normalized for token in {"made", "score", "swish"}):
        return "made"
    if event_family != "shot_attempt":
        return "uncertain"
    if ball_through_hoop is not None and ball_through_hoop >= 0.75:
        return "made"
    if hoop_visible and ball_near_rim is not None and ball_near_rim < 0.2 and ball_visible:
        return "missed"
    return "uncertain"


def _tracking_shot_subtype(label: str | None, *, event_family: str) -> str | None:
    if event_family != "shot_attempt":
        return None
    normalized = _normalize_label(label)
    ...
    if "putback" in normalized or "tip" in normalized:
        return "putback"
    if "jumper" in normalized or "midrange" in normalized or "pull up" in normalized:
        return "jumper"
...
def _bard_event_family(label: str | None) -> str:
    ...
    if not normalized:
        return "other"
    if any(token in normalized for token in {"dead ball", "deadball", "replay", "celebration", "inbound", "dribble only", "camera pan", "half court setup", "warmup", "walkthrough"}):
        return "other"
...
def _bard_outcome(label: str | None) -> str:
    ...
    if any(token in normalized for token in {"blocked"}):
        return "blocked"
    if any(token in normalized for token in {"turnover", "steal", "defense"}):
        return "uncertain"
    if any(token in normalized for token in {"miss", "rim out", "airball", "no good"}):
        return "missed"
...
def _bard_shot_subtype(label: str | None, *, event_family: str) -> str | None:
    ...
    if "jumper" in normalized or "midrange" in normalized:
        return "jumper"
    if "runner" in normalized or "floater" in normalized:
        return "layup"

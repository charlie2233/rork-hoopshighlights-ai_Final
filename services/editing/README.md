...
- `HOOPS_AI_CLIP_GPT_FREE_KEYFRAMES_PER_CLIP`: Free keyframe cap per clip, default `3` and clamped to `3...10`; staging/internal quality-beta sets `10` so Free testers get the same release, rim-entry, and follow-through evidence while Free daily edit chances stay at `3`
...
- `HOOPS_GPT_HIGHLIGHT_RERANK_FREE_MAX_CLIPS`: legacy Free cap override, clamped to `1...320`
- `HOOPS_GPT_HIGHLIGHT_RERANK_FREE_FRAMES_PER_CLIP`: legacy Free frames override, clamped to `3...10`
- `HOOPS_GPT_HIGHLIGHT_RERANK_PAID_MAX_CLIPS`: legacy Pro/internal cap, clamped to `20...320`
...
Quality-beta staging deploys set the GPT editor, plan edit, revision, and legacy reranker switches to `true` in `cloudbuild.yaml` and mount `HOOPS_OPENAI_API_KEY` from Secret Manager. Local and ad hoc environments still default those flags off unless explicitly configured.

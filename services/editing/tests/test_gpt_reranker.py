...
def _request(plan_tier: str = "free", clip_count: int = 10) -> CreateEditJobRequest:
    return CreateEditJobRequest(
        ...
        planTier=plan_tier,
        clips=[_clip(f"c{index}", float(index * 7), 1.0 - (index * 0.001)) for index in range(clip_count)],
    )
...
class GPTHighlightRerankerTests(unittest.TestCase):
    def test_payload_is_strict_structured_output_and_not_stored(self) -> None:
        ...
        self.assertIn("setup_event_result", compact_clip["qualityHints"]["actionContextGuidance"])
        self.assertIn("reject_late_fragment", compact_clip["qualityHints"]["actionContextGuidance"])
        ...
        self.assertTrue(compact_input["shotTrackerRules"]["requireSetupEventResultForKeptClip"])
        self.assertTrue(compact_input["shotTrackerRules"]["rejectLateFragmentOrPostPlayOnly"])
        self.assertTrue(compact_input["shotTrackerRules"]["rejectDeadBallScoreboardHuddleOnly"])
        self.assertIn("setup_event_result_follow_through", compact_input["selectionQualityRules"]["actionContextPolicy"])
        self.assertIn("outcomeEvidenceSource=label_only", payload["instructions"])
        self.assertIn("full action-to-result context", payload["instructions"])
        self.assertIn("Reject late fragments", payload["instructions"])
        self.assertIn("scoreboard-only/huddle-only", payload["instructions"])
    ...
    def test_payload_includes_team_targeting_and_excludes_confident_opponent_clips(self) -> None:
        ...
        self.assertEqual(compact_input["selectionQualityRules"]["selectedTeamRenderCandidateCount"], 1)
        self.assertEqual(compact_input["selectionQualityRules"]["selectedTeamEvidenceBackedCandidateCount"], 1)
        self.assertEqual(compact_input["selectionQualityRules"]["selectedTeamDefensiveCandidateCount"], 0)
        self.assertEqual(compact_input["selectionQualityRules"]["selectedTeamUncertainReviewCandidateCount"], 1)
        ...
        self.assertTrue(by_id["dark_make"]["teamDefenseContext"]["finalEditAllowed"])
        self.assertFalse(by_id["dark_make"]["teamDefenseContext"]["manualReviewRequired"])
        self.assertEqual(by_id["dark_make"]["teamDefenseContext"]["allowedFinalEditAction"], "render")
        self.assertEqual(by_id["dark_make"]["teamDefenseContext"]["renderEligibilityReason"], "selected_team_match")
        ...
        self.assertFalse(by_id["uncertain_make"]["teamDefenseContext"]["finalEditAllowed"])
        self.assertTrue(by_id["uncertain_make"]["teamDefenseContext"]["manualReviewRequired"])
        self.assertEqual(by_id["uncertain_make"]["teamDefenseContext"]["allowedFinalEditAction"], "review_only")
        self.assertEqual(by_id["uncertain_make"]["teamDefenseContext"]["renderEligibilityReason"], "needs_manual_team_review")
        ...
        self.assertIn("teamDefenseContext.candidateLane, finalEditAllowed, manualReviewRequired", payload["instructions"])
    ...
    def test_payload_adds_team_defense_context_for_selected_team_candidates(self) -> None:
        ...
        self.assertTrue(steal_context["finalEditAllowed"])
        self.assertFalse(steal_context["reviewOnlyUncertain"])
        self.assertFalse(steal_context["manualReviewRequired"])
        self.assertEqual(steal_context["allowedFinalEditAction"], "render")
        self.assertEqual(steal_context["renderEligibilityReason"], "selected_team_match")
        ...
        self.assertFalse(block_context["finalEditAllowed"])
        self.assertTrue(block_context["reviewOnlyUncertain"])
        self.assertTrue(block_context["manualReviewRequired"])
        self.assertEqual(block_context["allowedFinalEditAction"], "review_only")
        self.assertEqual(block_context["renderEligibilityReason"], "needs_manual_team_review")
    ...
    def test_payload_requires_shot_quality_signals_and_context_judgment(self) -> None:
        ...
        pro_frames = [
            SampledFrame(clip_id="c0", role=role, time_seconds=float(index), data_url="data:image/jpeg;base64,ZmFrZQ==")
            for index, role in enumerate(["start", *rich_shot_roles, "eventCenter", "finish"])
        ]
        pro_payload = _build_openai_payload(pro_request, pro_request.clips[:1], pro_frames, settings)
        pro_input = json.loads(pro_payload["input"][0]["content"][0]["text"])
        self.assertEqual(pro_input["shotTrackerRules"]["requiredShotContextKeyframes"], ["outcome", "preEvent", "release"])
        ...
        self.assertIn("setup or lead-in, basketball event, visible outcome, and follow-through", payload["instructions"])
        self.assertIn("reject clips that start right before the basket", payload["instructions"])
        self.assertIn("post-play-only aftermath", payload["instructions"])
    ...
    def test_revision_patch_payload_filters_selected_team_candidates(self) -> None:
        ...
        self.assertIn("teamEvidence.status=evidence_backed", payload["instructions"])
        self.assertIn("teamDefenseContext.allowedFinalEditAction is render", payload["instructions"])
    ...
    def test_gpt_sampling_reserves_multiple_audio_reactions_for_large_candidate_pool(self) -> None:
        ...
        self.assertGreaterEqual(len([clip_id for clip_id in sampled_ids if clip_id.startswith("crowd_pop_")]), 6)
        self.assertIn("crowd_pop_0", sampled_ids)

    def test_gpt_sampling_reserves_three_audio_reactions_for_free_pool(self) -> None:
        scoring = [
            _clip(f"score_{index}", float(index * 6), 0.99 - (index * 0.01))
        ...
        crowd_pops = [
            {
                **_labeled_clip(f"crowd_pop_free_{index}", 96.0 + float(index * 6), 0.66, "Crowd Reaction"),
                "audioPeak": 0.99 - (index * 0.01),
                "motionScore": 0.78,
                "watchability": 0.74,
                "excitement": 0.84,
        ...
        request = CreateEditJobRequest(
            videoId="video_audio_reaction_free_reserve",
            analysisJobId="analysis_audio_reaction_free_reserve",
            installId="install-123",
            sourceObjectKey="uploads/source.mp4",
            preset="personal_highlight",
            targetDurationSeconds=30,
        ...
        self.assertEqual(len([clip_id for clip_id in sampled_ids if clip_id.startswith("crowd_pop_free_")]), 3)
    ...
    def test_gpt_detects_ios_audio_pop_cue_label_as_recall_hint(self) -> None:
        request = CreateEditJobRequest(
            ...
            analysisJobId="analysis_ios_audio_pop_label",
            installId="install-123",
            sourceObjectKey="uploads/source.mp4",
            preset="personal_highlight",
            targetDurationSeconds=30,
            ...
            clips=[
                {
                    **_labeled_clip("ios_audio_pop_cue", 42.0, 0.58, "Audio Pop Cue - Action"),
                    "audioPeak": 0.96,
                    "motionScore": 0.52,
                    "watchability": 0.5,
                    "excitement": 0.72,
    ...
    def test_gpt_detects_super_loud_audio_pop_with_some_action_context(self) -> None:
        request = CreateEditJobRequest(
            videoId="video_super_loud_audio_pop",
            analysisJobId="analysis_super_loud_audio_pop",
            installId="install-123",
            sourceObjectKey="uploads/source.mp4",
            preset="personal_highlight",
            targetDurationSeconds=30,
            ...
            clips=[
                {
                    **_labeled_clip("super_loud_pop", 54.0, 0.46, "Highlight"),
                    "audioPeak": 0.99,
                    "motionScore": 0.5,
                    "watchability": 0.41,
                    "excitement": 0.5,
    ...
    def test_gpt_caption_sanitizer_replaces_plan_caption_that_conflicts_with_outcome(self) -> None:
        request = CreateEditJobRequest(
            videoId="video_caption_guard",
            analysisJobId="analysis_caption_guard",
            installId="install-123",
            preset="personal_highlight",
            targetDurationSeconds=30,
            planTier="free",
        ...
        plan_edit = GPTPlanEdit(
            orderedClipIds=["steal"],
            captions=[{"clipId": "steal", "caption": "BUCKET", "captionMoment": 13.0}],
        )
        ...
        self.assertEqual(result.gptRerankSummary.storyOrderClipIds, ["steal"])
        self.assertTrue(result.gptRerankSummary.planEditApplied)

    def test_gpt_caption_sanitizer_keeps_non_conflicting_defensive_caption(self) -> None:
        request = CreateEditJobRequest(
            videoId="video_caption_keep",
            analysisJobId="analysis_caption_keep",
            installId="install-123",
            preset="personal_highlight",
            targetDurationSeconds=30,
            planTier="free",
        ...
        decision = GPTHighlightClipDecision(
            ...
            highlightScore=0.9,
            watchabilityScore=0.84,
            basketballEvent="Defensive Stop",
    ...
    def test_free_and_pro_sampling_limits(self) -> None:
        ...
        self.assertEqual(public_status["ai_clip_gpt_max_candidates_free"], 8)
        self.assertEqual(public_status["ai_clip_gpt_max_candidates_pro"], 30)
        self.assertEqual(public_status["ai_clip_gpt_max_candidates_internal"], 30)
        self.assertEqual(public_status["ai_clip_gpt_keyframes_per_clip"], {"free": 3, "pro": 6, "internal": 6})

    def test_free_default_keyframes_use_compact_start_center_finish_context(self) -> None:
        settings = GPTHighlightRerankerSettings.from_env()
        ...
        self.assertEqual(frames_per_clip, 3)
        self.assertEqual(
            {
                "start",
                "eventCenter",
    ...
    def test_free_sampling_reviews_compact_top_pool_by_default(self) -> None:
        settings = GPTHighlightRerankerSettings.from_env()
    ...
    def test_sampling_caps_are_applied_before_openai_call(self) -> None:
        ...
        try:
            gpt_reranker._extract_candidate_keyframes = fake_extract
            with tempfile.NamedTemporaryFile(suffix=".mp4") as source:
                free_result = gpt_reranker.rerank_edit_request_with_gpt(_request("free", 160), Path(source.name), settings, fake_response_client)
                pro_result = gpt_reranker.rerank_edit_request_with_gpt(_request("pro", 160), Path(source.name), settings, fake_response_client)
        finally:
            gpt_reranker._extract_candidate_keyframes = original_extract

        self.assertEqual(observed_calls, [(160, 3), (160, 5)])
        self.assertEqual(len(free_result.gptRerankSummary.storyOrderClipIds), 160)
        self.assertEqual(len(pro_result.gptRerankSummary.storyOrderClipIds), 160)

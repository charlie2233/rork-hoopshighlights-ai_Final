...
- Filter out tiny, too-late, or no-follow-through candidates before GPT; require GPT to return strict shot-quality signals plus explicit `shotResultEvidence` for release-to-rim continuity, visible rim/net result, outcome confidence, camera quality, and full play context.

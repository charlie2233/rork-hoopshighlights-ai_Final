...
Before upload, the iOS control surface automatically prepares a balanced 720p analysis source for recordings that are at least 12 minutes or 256 MiB. Shorter, smaller sources keep their original bytes, and an optimized export is used only when it saves at least 18 percent. Fast Upload Mode forces source preparation; recordings at least 45 minutes or 1.4 GiB prefer the compact 540p profile. This changes transfer size only: detection, edit planning, and rendering remain cloud-owned.

...
class BuildTeamHighlightLabelReviewPageTests(unittest.TestCase):
    def test_builds_local_review_page_without_raw_urls_or_object_keys(self) -> None:
        ...
        self.assertIn("Launch evidence checklist", page)
        self.assertIn("0</strong> / 1 clips reviewed", page)
        self.assertIn("1</strong> clips remain", page)
        self.assertIn("Watch the source video before marking a clip reviewed", page)
        self.assertIn("expected.teamId", page)
        self.assertIn("expected.isHighlight", page)
        self.assertIn("expected.eventType", page)
        self.assertIn("expected.outcome", page)
        self.assertIn("needsLabel=false", page)
        self.assertIn("reviewedByHuman=true", page)
        self.assertIn("progress checkpoints are data-entry help only", page)
        ...
        self.assertIn("Click card or Enter/Space plays the clip window", page)
        self.assertIn("J/L scrub", page)
        self.assertIn("Click any clip card or Play clip to jump to its start; playback auto-pauses at the clip end.", page)
        ...
        self.assertIn("playClipWindowFromCard", page)
        self.assertIn("handleClipCardClick", page)
        self.assertIn("handleClipEndTimeUpdate", page)
        self.assertIn("targetIsClipControl", page)
        self.assertIn("auto-pause", page)
        self.assertIn("paused at clip end", page)
        self.assertIn('window.addEventListener("click", handleClipCardClick)', page)
        self.assertIn('video.addEventListener("timeupdate", handleClipEndTimeUpdate)', page)
        self.assertIn("Playing synced angles.", page)
        self.assertIn('[" ", "enter", "s", "e", "f", "j", "k", "l", "p", "r", "n", "1", "2", "3"].includes(key)', page)
        ...
        self.assertIn(".clip-card.playing", page)
        self.assertIn(".playback-status", page)
        ...
        self.assertLess(page.index("Next close review"), page.index("Next incomplete"))
        self.assertIn("Review order:", page)
        self.assertIn("start with Close review clips", page)
        self.assertIn("Quick checks still need the video watched", page)

...
"""
    ...
    def test_installed_testflight_build_passes_for_build_55(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            repo_root = Path(temp_dir)
        ...
        self.assertFalse(has_failures(collector.findings))
        self.assertIn("1.0.0 (55)", collector.findings[0].detail)
    ...
    def test_installed_testflight_build_fails_for_old_build(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            payload = {
                ...
                "blockers": ["Build number mismatch: expected 55, got 49."],
            }
        ...
        self.assertIn("Install/update build 55", detail)
    ...
    def test_upload_artifact_accepts_duplicate_upload_when_status_proves_internal_testing(self) -> None:
        ...
        status_log = "App Store Connect confirms this build is ready for internal TestFlight."
        duplicate_log = "The bundle version must be higher than the previously uploaded version: ‘55’."

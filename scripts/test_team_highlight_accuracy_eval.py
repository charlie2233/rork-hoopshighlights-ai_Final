...
def defensive_outcome_evidence(outcome: str = "steal", outcome_confidence: float = 0.86) -> dict:
    return {
        "outcome": outcome,
        "qualitySignals": {
            "setupVisible": True,
    ...
    return [
        ...
        },
        {
            ...
            "prediction": timed_prediction(
                {
                    "keep": True,
                    "teamAttribution": team_attribution(0.9),
                    **defensive_outcome_evidence("forced_turnover"),
                },
                start=40.0 + offset,
                end=43.2 + offset,
        ...
        {
            "expected": {"teamId": "team_dark", "isHighlight": True, "eventType": "defensive stop"},
            "prediction": timed_prediction(
                {
                    "keep": True,
                    "teamAttribution": team_attribution(0.9),
                    **defensive_outcome_evidence("defensive_stop"),
                },
                start=46.0 + offset,
                end=49.2 + offset,
                event_center=47.4 + offset,
            ),
        },
        {
...
def all_teams_coverage_clips(offset: float = 0.0) -> list[dict]:
    return [
        {
            "expected": {"teamId": "team_dark", "isHighlight": True, "eventType": "made_three"},
            "prediction": timed_prediction(
        ...
        {
            "expected": {"teamId": "team_light", "isHighlight": True, "eventType": "made_layup", "outcome": "made"},
            "prediction": timed_prediction(
                {
                    "keep": True,
                    "teamAttribution": team_attribution(0.93, team_id="team_light"),
                    **made_shot_evidence(),
                },
                start=20.0 + offset,
                end=24.0 + offset,
                event_center=22.0 + offset,
            ),
        },
        {
...
class TeamHighlightAccuracyEvalTests(unittest.TestCase):
    def test_selected_team_eval_counts_uncertain_review_and_defensive_events(self) -> None:
        report = evaluate_accuracy(
            {
                ...
                "cases": [
                    ...
                    },
                    {
                        ...
                        "clips": all_teams_coverage_clips(offset=200.0),
                    },
                ]
            }
        )
        ...
        self.assertEqual(report.metrics.clipCount, 18)
        ...
        self.assertEqual(report.metrics.selectedTeamHighlightCount, 10)
        self.assertEqual(report.metrics.defensiveEventCount, 8)
        self.assertEqual(report.metrics.shotOutcomeEvidenceClipCount, 13)
        ...
        self.assertEqual(report.metrics.selectedTeamForcedTurnoverCount, 2)
        self.assertEqual(report.metrics.selectedTeamDefensiveStopCount, 2)
        self.assertEqual(report.metrics.opponentHighlightCount, 2)
        self.assertEqual(report.metrics.negativeClipCount, 3)
        self.assertEqual(report.metrics.badWindowNegativeCount, 3)
        self.assertEqual(report.metrics.selectedTeamEvidenceClipCount, 8)
        ...
        self.assertEqual(report.evidence.distinctVideoCount, 3)
    ...
    def test_default_readiness_requires_all_teams_coverage(self) -> None:
        report = evaluate_accuracy(
            {
                "cases": [
                    {
    ...
    def test_defensive_outcome_low_confidence_fails_outcome_quality(self) -> None:
        report = evaluate_accuracy(
            {
                "selectedTeamId": "team_dark",
                "clips": [
                        ...
                        "prediction": timed_prediction(
                            {
                                "keep": True,
                                "teamAttribution": team_attribution(0.94),
                                **defensive_outcome_evidence("steal", outcome_confidence=0.41),
                            },
                            start=30.0,
                            end=33.2,
                            event_center=31.4,
            ...
            thresholds=AccuracyThresholds(
                ...
                shotOutcomeEvidenceQuality=1.0,
                minCases=1,
                minScoredClips=1,
        ...
        report = evaluate_accuracy(
            {
                "cases": [
                    {
                        "caseId": "game_all_defense",
                        "videoId": "video_all_defense",
                        "analysisJobId": "analysis_all_defense",
                        "teamScanJobId": "scan_all_defense",
                        "teamMode": "all",
                        "detectedTeams": [
                            {"teamId": "team_dark", "label": "Black jerseys", "colorLabel": "black", "confidence": 0.93},
                            {"teamId": "team_light", "label": "White jerseys", "colorLabel": "white", "confidence": 0.91},
                        ...
                        "clips": [
                            {
                                "expected": {"teamId": "team_light", "isHighlight": True, "eventType": "block"},
                                "prediction": timed_prediction(
                                    {
                                        "keep": True,
                                        "teamAttribution": {"teamId": "team_light", "confidence": 0.9},
                                        **blocked_shot_evidence(),
                                    },
                                    start=20.0,
                                    end=23.2,
    ...
    def test_default_readiness_requires_forced_turnover_and_defensive_stop_coverage(self) -> None:
        def without_forced_or_stop(offset: float = 0.0) -> list[dict]:
            return [
                clip
                for clip in readiness_coverage_clips(offset=offset)
                if clip["expected"].get("eventType") not in {"forced turnover", "defensive stop"}
            ]
        ...
        report = evaluate_accuracy(
            {
                "cases": [
                    ...
                    },
                    {
                        ...
                        "clips": without_forced_or_stop(offset=100.0),
                    },
                    {
                        "caseId": "game_all_001",
                        "videoId": "video_all_001",
        ...
        self.assertEqual(report.status, "fail")
        self.assertTrue(any("selectedTeamForcedTurnoverCoverage" in failure for failure in report.failures))
        self.assertTrue(any("selectedTeamDefensiveStopCoverage" in failure for failure in report.failures))
    ...
    def test_made_shot_without_visible_rim_entry_evidence_fails_outcome_quality(self) -> None:
        ...
        self.assertEqual(report.status, "fail")
        self.assertEqual(report.metrics.shotOutcomeEvidenceClipCount, 3)
        self.assertEqual(report.metrics.badShotOutcomeEvidenceCount, 1)
        self.assertTrue(any("shotOutcomeEvidenceQuality" in failure for failure in report.failures))

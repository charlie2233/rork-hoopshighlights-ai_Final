...
class PrepareTeamHighlightLabelingBundleTests(unittest.TestCase):
    def test_prepares_review_page_status_and_next_steps_with_multi_angle(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            review_queue = (output_dir / "review_queue.md").read_text()
        ...
        self.assertIn("HoopClips Human Label Review Queue", review_queue)
        self.assertIn("label_001", review_queue)
        self.assertIn("reviewer navigation help only", review_queue)
        self.assertIn("apply_team_highlight_manual_labels.py", next_steps)
        self.assertIn("Review queue", next_steps)
        self.assertIn("export_team_highlight_review_queue.py", next_steps)
        self.assertIn("Review queue rows: `1` incomplete clips", next_steps)
        self.assertIn("navigation help only, not evidence", next_steps)
        ...
        self.assertIn("Launch Evidence Checklist", next_steps)
        self.assertIn("Reviewed clips: `0` of `1`", next_steps)
        self.assertIn("Remaining clips: `1`", next_steps)
        self.assertIn("expected.teamId", next_steps)
        self.assertIn("expected.isHighlight", next_steps)
        self.assertIn("expected.eventType", next_steps)
        self.assertIn("expected.outcome", next_steps)
        self.assertIn("final bundle is applied without `--allow-incomplete`", next_steps)

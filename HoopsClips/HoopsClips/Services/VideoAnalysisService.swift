...
final class VideoAnalysisService {
    ...
    func buildAudioOnlyClips(audioPeaks: [Double], duration: Double) -> [Clip] {
        guard !audioPeaks.isEmpty, duration > 0 else { return [] }

        let maxPeak = audioPeaks.max() ?? 0.0
        guard maxPeak > 0.05 else { return [] }

        let thresholds = [0.3 * maxPeak, 0.15 * maxPeak]
        for threshold in thresholds {
            let candidates = buildBoundedAudioFallbackCandidates(
                audioPeaks: audioPeaks,
                duration: duration,
                threshold: threshold
            )
    ...
    func normalizeDetectedClips(_ detectedClips: [Clip], duration: Double) -> [Clip] {
        guard duration > 0 else { return [] }

        var expanded: [Clip] = []
        expanded.reserveCapacity(detectedClips.count)

        for clip in detectedClips {
            guard let bounded = clampedClip(clip, duration: duration) else { continue }
            if isOverlongClip(bounded, duration: duration) {
                expanded.append(contentsOf: splitOverlongClip(bounded, duration: duration))
            } else {
                expanded.append(bounded)
            }
        }

        guard !expanded.isEmpty else { return [] }

        let ranked = expanded.sorted { lhs, rhs in
            if lhs.combinedScore == rhs.combinedScore {
        ...
        var deduplicated: [Clip] = []
        for clip in ranked {
            if deduplicated.contains(where: { overlapRatio(between: clip, and: $0) > 0.60 }) {
                continue
            }
    ...
    private func buildBoundedAudioFallbackCandidates(
        ...
        let activeIndices = audioPeaks.indices.filter { audioPeaks[$0] > threshold }
        guard !activeIndices.isEmpty else { return [] }

        let baseWindowLength = max(
            max(2.0, settings.minClipDuration),
            min(6.0, settings.maxClipDuration * 0.6)
        )
        let spacingBuckets = max(Int(ceil(baseWindowLength * 0.6 * 10.0)), 1)

        var clusters: [[Int]] = []
        for index in activeIndices {
            if let lastIndex = clusters.last?.last,
               Double(index - lastIndex) / 10.0 <= 1.2 {
        ...
        var rankedCandidates: [(clip: Clip, score: Double)] = []
        for cluster in clusters {
            guard let first = cluster.first, let last = cluster.last else { continue }
            let clusterSpan = Double(last - first) / 10.0
            let maxCandidates: Int
            if clusterSpan < 8.0 {
                maxCandidates = 1
            } else if clusterSpan < 20.0 {
                maxCandidates = 2
            } else {
                maxCandidates = 3
            }

            let rankedIndices = cluster.sorted { lhs, rhs in
                if audioPeaks[lhs] == audioPeaks[rhs] {
                    return lhs < rhs
                }
                return audioPeaks[lhs] > audioPeaks[rhs]
            }

            var selectedCenters: [Int] = []
            for index in rankedIndices {
                if selectedCenters.contains(where: { abs($0 - index) < spacingBuckets }) {
                    continue
                }
                selectedCenters.append(index)
                if selectedCenters.count == maxCandidates {
                    break
                }
            }

            let clusterPeak = cluster.map { audioPeaks[$0] }.max() ?? 0.0
            for centerIndex in selectedCenters {
                let centerTime = Double(centerIndex) / 10.0
                guard let window = boundedWindow(
                    center: centerTime,
                    preferredLength: baseWindowLength,
                    minLength: max(2.0, settings.minClipDuration),
                    lowerBound: 0.0,
                    upperBound: duration
                ) else {
                    continue
                ...
                let localAverage = AnalysisQualityTuning.localAverage(values: audioPeaks, center: centerIndex, radius: 2)
                let ranking = min(1.0, max(0.0, (localAverage * 0.65) + (clusterPeak * 0.35)))
                let confidence = min(0.52, max(0.38, 0.38 + ranking * 0.14))
        ...
        let deduplicated = rankedCandidates
            .sorted {
                if $0.score == $1.score {
            ...
            .reduce(into: [Clip]()) { partialResult, candidate in
                if partialResult.contains(where: { overlapRatio(between: candidate.clip, and: $0) > 0.60 }) {
                    return
                }
                partialResult.append(candidate.clip)
            }

        return deduplicated.sorted { $0.startTime < $1.startTime }
    }

    private func clampedClip(_ clip: Clip, duration: Double) -> Clip? {
        var bounded = clip
    ...
    private func isOverlongClip(_ clip: Clip, duration: Double) -> Bool {
        clip.duration > settings.maxClipDuration
            || (duration >= 20.0 && clip.duration > duration * 0.9)
    }

    private func splitOverlongClip(_ clip: Clip, duration: Double) -> [Clip] {
        let minLength = max(2.0, settings.minClipDuration)
        ...
        while cursor < clip.endTime && derived.count < 3 {
            let center = min(
                max(cursor + (segmentLength / 2.0), clip.startTime + (segmentLength / 2.0)),
                clip.endTime - min(segmentLength, clip.duration) / 2.0
            )
            if let window = boundedWindow(
                center: center,
                ...
                var segment = clip
                segment.startTime = window.start
                segment.endTime = window.end
        ...
        if derived.isEmpty,
           let window = boundedWindow(
               center: (clip.startTime + clip.endTime) / 2.0,
               preferredLength: segmentLength,
               minLength: minLength,
               lowerBound: clip.startTime,
               upperBound: min(duration, clip.endTime)
    ...
    private func boundedWindow(
        center: Double,
        preferredLength: Double,
        minLength: Double,
        lowerBound: Double,
        upperBound: Double
    ) -> (start: Double, end: Double)? {
        let boundedUpper = max(lowerBound, upperBound)
        let availableLength = boundedUpper - lowerBound
        guard availableLength > 0 else { return nil }

        let effectiveMinLength = min(minLength, availableLength)
        let targetLength = min(max(preferredLength, effectiveMinLength), availableLength)
        guard targetLength > 0 else { return nil }

        var start = center - (targetLength / 2.0)
        var end = start + targetLength

        if start < lowerBound {
            start = lowerBound
            end = min(boundedUpper, start + targetLength)
        }

        if end > boundedUpper {
            end = boundedUpper
            start = max(lowerBound, end - targetLength)
        }

        if end - start < effectiveMinLength {
            start = max(lowerBound, end - effectiveMinLength)
            end = min(boundedUpper, start + effectiveMinLength)
        }
    ...
    private func overlapRatio(between lhs: Clip, and rhs: Clip) -> Double {
